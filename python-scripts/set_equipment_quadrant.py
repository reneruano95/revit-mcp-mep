...
PARAM_ROOM_QUADRANT = "JAL Location"  # Parameter to store NW, SW, SE, NE
...
if not linked_doc:
    ...
    building_center_y = (min_y + max_y) / 2

    print(f"Building center point: X={building_center_x}, Y={building_center_y}")
        ...
        if eq_level_id in rooms_by_level:
            for room in rooms_by_level[eq_level_id]:
                if room.IsPointInRoom(transformed_point):
                    ...
                    # Determine quadrant based on room location
                    room_center = room.Location.Point
                    quadrant = ""
                    if (
                        room_center.X >= building_center_x
                        and room_center.Y >= building_center_y
                    ):
                        quadrant = "NE"
                    elif (
                        room_center.X >= building_center_x
                    ...
                    # Set the quadrant parameter
                    quadrant_param = eq.LookupParameter(PARAM_ROOM_QUADRANT)
                    if quadrant_param:
                        quadrant_param.Set(quadrant)
                        print(
                            f"Set quadrant to {quadrant} for equipment in room {room_number}"
                        )
                        updated_count += 1
                    else:
                        print(
                            f"Missing '{PARAM_ROOM_QUADRANT}' parameter on equipment: {eq.Id}"
                        )
                        error_count += 1
